...
```{r,echo=TRUE}
library(glmnet)
x <- data.matrix(ames_raw_2008[, c(14,19,48)])
y <- log(ames_raw_2008$SalePrice)

# Perform cross-validation to determine the optimal lambda for regularization
cross_validated <- cv.glmnet(x, y, alpha = 1)
...
ridge <- glmnet(x, y, alpha = 1, lambda = lambda)

newx <- data.matrix(ames_raw_2009[, c(14,19,48)])
lasso_predicted <- predict(ridge, newx = newx)

lasso_loss <- calc_loss(exp(lasso_predicted), ames_raw_2009$SalePrice)
print("Lasso Loss")
print(lasso_loss)
```
...
# Generate a predictor vector X
X <- rnorm(n)

# Generate a noise vector epsilon
e <- rnorm(n)
...
Your code:
```{r,echo=TRUE}
# Assuming that X and epsilon have already been generated as per the previous part
# Set the seed for reproducibility
set.seed(123)
...
data <- data.frame(Y = Y, X = X, X2 = X^2, X3 = X^3, X4 = X^4, X5 = X^5, 
                   X6 = X^6, X7 = X^7, X8 = X^8, X9 = X^9, X10 = X^10)

# Perform best subset selection
subset_selection <- regsubsets(Y ~ ., data=data, nbest=1, nvmax=10, method="exhaustive")

# Get summary of best subset selection
subset_summary <- summary(subset_selection)

# Identify the best model according to Cp, BIC, and adjusted R^2
# Note: This step is for illustration. In practice, you might want to look at each criterion separately.
best_by_cp <- which.min(subset_summary$cp)
best_by_bic <- which.min(subset_summary$bic)
best_by_adjr2 <- which.max(subset_summary$adjr2)

# Extract the coefficients of the best model by one of the criteria, for example, BIC
best_model <- coef(subset_selection, id=best_by_bic)

# Plotting
plot(subset_summary$cp, xlab="Number of Variables", ylab="Cp", type='l')
points(best_by_cp, subset_summary$cp[best_by_cp], col="red", cex=2, pch=20)

plot(subset_summary$bic, xlab="Number of Variables", ylab="BIC", type='l')
points(best_by_bic, subset_summary$bic[best_by_bic], col="red", cex=2, pch=20)

plot(subset_summary$adjr2, xlab="Number of Variables", ylab="Adjusted R^2", type='l')
points(best_by_adjr2, subset_summary$adjr2[best_by_adjr2], col="red", cex=2, pch=20)

# Output the coefficients of the best model
best_model
...
forward_selection <- regsubsets(Y ~ ., data=data, nbest=1, nvmax=10, method="forward")
forward_summary <- summary(forward_selection)

# Perform backward stepwise selection
backward_selection <- regsubsets(Y ~ ., data=data, nbest=1, nvmax=10, method="backward")
...
predictors <- as.matrix(data[, -1])  # exclude the response variable Y
response <- data$Y
...
# Find the lambda that gives minimum mean cross-validated error
optimal_lambda <- cv_lasso$lambda.min

# Fit lasso model on the full data with the selected lambda
lasso_model <- glmnet(predictors, response, alpha=1, lambda=optimal_lambda)

# Get the coefficient estimates
lasso_coefficients <- coef(lasso_model)

# Output the optimal lambda and coefficients
list(optimal_lambda = optimal_lambda, lasso_coefficients = lasso_coefficients)
...
```{r,echo=TRUE}
# Generate the response vector Y
beta0 <- 1 # replace with your chosen value
beta7 <- 2 # replace with your chosen value
Y <- beta0 + beta7 * X^7 + e

# Create the data frame for predictors X, X^2, ..., X^10
data <- data.frame(Y = Y, X = X, X2 = X^2, X3 = X^3, X4 = X^4, X5 = X^5,
                   X6 = X^6, X7 = X^7, X8 = X^8, X9 = X^9, X10 = X^10)
...
training_set <- College[train_indices, ]
...
Your code:
```{r,echo=TRUE}
# Assuming the 'training_set' and 'test_set' have been created as shown earlier

# Fit a linear model using all other variables to predict 'Apps'
# 'Apps' is assumed to be the response variable you want to predict
# Replace 'Apps' with the actual name of the response variable if it's different
linear_model <- lm(Apps ~ ., data = training_set)

# Predict on the test set
predictions <- predict(linear_model, newdata = test_set)

# Calculate the test error (MSE)
test_error <- mean((test_set$Apps - predictions)^2)
...
Your code:
```{r,echo=TRUE}
training_predictors <- model.matrix(Apps ~ . - 1, data = training_set)
test_predictors <- model.matrix(Apps ~ . - 1, data = test_set)
training_response <- training_set$Apps

# Fit ridge regression model using cross-validation to find optimal lambda
cv_ridge <- cv.glmnet(training_predictors, training_response, alpha = 0, lambda = 10^seq(10, -2, length = 100))
optimal_lambda <- cv_ridge$lambda.min
ridge_model <- glmnet(training_predictors, training_response, alpha = 0, lambda = optimal_lambda)

# Predict on the test set
ridge_predictions <- predict(ridge_model, s = optimal_lambda, newx = test_predictors)

ridge_test_error <- mean((test_set$Apps - ridge_predictions)^2)
ridge_test_error
```
...
cv_lasso <- cv.glmnet(training_predictors, training_response, alpha = 1)
optimal_lambda <- cv_lasso$lambda.min
lasso_model <- glmnet(training_predictors, training_response, alpha = 1, lambda = optimal_lambda)

lasso_predictions <- predict(lasso_model, s = optimal_lambda, newx = test_predictors)

lasso_test_error <- mean((test_set$Apps - lasso_predictions)^2)

non_zero_coefficients <- sum(coef(lasso_model, s = optimal_lambda) != 0)
list(test_error = lasso_test_error, non_zero_coefficients = non_zero_coefficients)
```
...
response <- training_set$Apps
predictors <- training_set[, -which(names(training_set) == "Apps")]

# Fit the PCR model using cross-validation to select the number of components
pcr_model <- pcr(Apps ~ ., data = training_set, scale = TRUE, validation = "CV")

# Access the cross-validation results directly from the pcr_model object
validation_results <- pcr_model$validation

# If you want to extract the MSE for each number of components and find the minimum
optimal_components <- which.min(validation_results$PRESS)

# Now use the optimal number of components to predict and calculate the test error
final_pcr_model <- pcr(Apps ~ ., data = training_set, scale = TRUE, ncomp = optimal_components)

# Predict on the test set
test_predictors <- test_set[, -which(names(test_set) == "Apps")]
pcr_predictions <- predict(final_pcr_model, newdata = test_predictors, ncomp = optimal_components)

# Calculate the test error (MSE)
pcr_test_error <- mean((test_set$Apps - pcr_predictions)^2)

# Report the test error and the optimal number of components
list(test_error = pcr_test_error, optimal_components = optimal_components)
```
...
# Fit the PLS model using cross-validation to select the number of components
pls_model <- plsr(Apps ~ ., data = training_set, scale = TRUE, validation = "CV")

validation_results <- pls_model$validation

# If you want to extract the MSE for each number of components and find the minimum
optimal_components <- which.min(validation_results$PRESS)

# Now use the optimal number of components to predict and calculate the test error
final_pls_model <- plsr(Apps ~ ., data = training_set, scale = TRUE, ncomp = optimal_components)

# Predict on the test set
test_predictors <- test_set[, -which(names(test_set) == "Apps")]
pcr_predictions <- predict(final_pls_model, newdata = test_predictors, ncomp = optimal_components)
...
names(df_X) <- paste("feature", 1:p, sep = "_")
df_y <- data.frame(response = y)
...
set.seed(0)
training_indices <- sample(1:nrow(df_X), training_size)

training_X <- df_X[training_indices, ]
training_y <- df_y[training_indices, ]

test_X <- df_X[-training_indices, ]
test_y <- df_y[-training_indices, ]

cat("Training set dimensions (X):", dim(training_X), "\n")
cat("Test set dimensions (X):", dim(test_X), "\n")
...
lasso_regression <- function(data) {
  ...
  return(list(cv = cv_out, model = lasso_mod))
}

# Principal component regression (PCR)
pcr_regression <- function(data) {
  pcr_mod <- pcr(crim ~ ., data = Boston, scale = TRUE, validation = "CV")
  summary(pcr_mod)
}

# Now apply these methods
best_subset <- best_subset_selection(Boston)
ridge <- ridge_regression(Boston)
lasso <- lasso_regression(Boston)
pcr <- pcr_regression(Boston)
```
...
Your code:
```{r,echo=TRUE}
# Performing best subset selection using the regsubsets function
best_subset_model <- regsubsets(crim ~ ., data=Boston, nvmax=NULL, method="exhaustive")
best_subset_summary <- summary(best_subset_model)

# Choose the model with the lowest BIC
# BIC tends to penalize complex models more than AIC
# which might lead to a better-performing model in terms of prediction error
best_model <- which.min(best_subset_summary$bic)
...
# Principal component regression with cross-validation
pcr_model <- pcr(crim ~ ., data=Boston, scale=TRUE, validation="CV")
validationplot(pcr_model, val.type="MSEP")


# Assume we have MSEP values from cross-validation for each model
mse_ridge <- mean(cv_ridge$cvm)
mse_lasso <- mean(cv_lasso$cvm)
mse_pcr <- mean(summary(pcr_model)$validation$MSEP[1,])
